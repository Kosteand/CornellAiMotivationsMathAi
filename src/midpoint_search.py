...
def certify_candidate(x_star, target_hits=100, lo=0.40, hi=0.60, alpha=0.05,
                        max_runs=300, fixed_kwargs=None, verbose=True):
    """
    ...
    anytime-valid confidence sequence. This is where the actual statistical
    guarantee lives; nothing from the search phase feeds into it.

    Each run trains from scratch at x_star and evaluates via
    run_eval_until_target_hits (through run_training), giving
    Y_hat = left / (left + right) -- misses are excluded from both
    numerator and denominator, matching the convention used everywhere
    ...
    while n < max_runs:
        ...
        mean = sum(ys) / n
        if n >= 2:
            s2 = sum((yi - mean) ** 2 for yi in ys) / (n - 1)
            s2 = min(max(s2, 1e-6), 0.25)
