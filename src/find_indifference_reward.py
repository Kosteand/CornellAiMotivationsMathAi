...
against ComplexityGroup, HeatmapGroup, or any other BanditEnv-compatible
...
def _default_group_factory(g, difficulty, value):
    ...
    MarginGroup-only behavior exactly: `difficulty` (what every docstring
    here still calls "k") maps to `delta = 1.0 / difficulty`. Pass a
    different `group_factory(g, difficulty, value) -> Group` to
    find_candidate_reward()/certify_reward()/find_indifference_reward()
    to use a different Group type entirely - see their docstrings'
    "group_factory" note."""
    return MarginGroup(g=g, delta=1.0 / difficulty, value=value)
...
def _build_groups(g, k_fixed, k_variable, value_fixed, value_variable, group_factory=None):
    ...
    pipeline against ComplexityGroup, HeatmapGroup, or any other
...
def find_candidate_reward(
    ...
    building ComplexityGroup or HeatmapGroup instead) is a drop-in swap.
...
def find_indifference_reward(
    ...
    complete swap: e.g. a `lambda g, k, value: ComplexityGroup(g=g, k=k,
    value=value)` runs this whole pipeline against ComplexityGroup
