...
and make PAIRS entries whatever shape that factory expects - e.g.:
    ...
    from Utilities.bandit_env import ComplexityGroup, HeatmapGroup

    # ComplexityGroup: difficulty is k, same as MarginGroup's default,
    # just a different Group subclass.
    GROUP_FACTORY = lambda g, k, value: ComplexityGroup(g=g, k=k, value=value)
    PAIRS = ((1, 3), (3, 13))
...
def mixed_group_factory(g, spec, value):
    ...
    if kind == "margin":
        if len(params) == 1:
            ...
            k, s, err = params
        return MarginGroup(g=g, delta=s / k, value=value, s=s, err=err)
    if kind == "heatmap":
