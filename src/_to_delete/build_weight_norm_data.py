"""One-off, already-run historical build script: combines the four raw
weight-norm-sweep CSVs into the unified schema and writes
weight_norm_data.csv. Kept only for provenance - this legacy file/schema
is superseded by group_data.py's group_data.csv (see legacy_csv_data.py
for read-only parsing of the old weight_norm_data.csv, if needed); new
per-group training runs should go through run_pipeline.py /
run_my_comparisons.py instead of appending to the old schema.
"""
import pandas as pd
